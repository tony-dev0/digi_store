...
function Legend({ items, onToggle }: any) {
  return (
    <Stack
      direction="row"
      sx={{
...
export default function IncomeAreaChart({ view }: any) {
  ...
  const toggleVisibility = (label: string | number) => {
    setVisibility((prev: any) => ({ ...prev, [label]: !prev[label] }))
  }
      ...
      >
        <defs>
          <linearGradient id="myGradient1" gradientTransform="rotate(90)">
            <stop offset="10%" stopColor={alpha('#8280ae', 0.4)} />
            <stop offset="90%" stopColor={alpha('#e9e8f0', 0.4)} />
          </linearGradient>
