    ...
    const handleFileDrop3 = (event: React.DragEvent<HTMLDivElement>) => {
      ...
      const handleFileDrop4 = (event: React.DragEvent<HTMLDivElement>) => {
        ...
        const onSubmit: SubmitHandler<FormFields> = async (data: FormFields) => {
          ...
          // it's better never to specify headers when using formdata since it is handled by formData
      ...
      const Popup = () => {
        toast.error("no actions for now");
      }
  return (
    <div className="px-2">
      ...
      <div className="w-100 d-flex justify-content-end mb-3">
        <button className='py-2 px-2 btn btn-customx' onClick={handleShow}>
          <LoupeIcon /> Add New Product</button>
      </div>
                  ...
                  <div className="f_item text-center"> 
                      <img src={require("../../assets/"+product.photos[0])} />
                        ...
                        <div className="d-flex align-items-center justify-content-center">
                            <button className="px-5 py-2 my-2 btn rounded-3 btn-sm btn-customx" onClick={Popup}> View Item</button>                   
                        </div>
                    </div>   
                  </div>
              }})}
  ...
  <section className="top_items py-5" id="product">
      ...
      <div className="wrapper-contain-item">
            ...
            if (product.category == "top"){
                  return <div className="item mtl" key={i} style={{height:'320px'}}>
               ...
               <div style={{width:'100%', background:'#ffffff'}}>
                  ...
                  <div className="b_text px-3">
                      <div className="productTitle">
                        <h6 className="mt-3">{(product.name).length >= 40 ? (product.name).slice(0,35)+"..." : product.name}</h6>
                      </div>  
                      <div className="priceSection mb-2">
                        <h5 className="text-muted p-0 mb-1">{usd.format(product.price)}</h5>
                        <del><h6 className="text-muted p-0">{product.prevprice ? usd.format(product.prevprice) : <span>&nbsp;</span>}</h6></del>
                        </div>
                      </div>
                  </Link>
                <div className="d-flex align-items-center mb-2 justify-content-center">
                    <Link to={`/admin/products/desc/${product._id}`}>
                        <button className="px-5 py-2 mb-2 btn rounded-3 btn-sm btn-customx"> View Item</button>
                    </Link>
                </div>
            </div> }})}
      </div>
