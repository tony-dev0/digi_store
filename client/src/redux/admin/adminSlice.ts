...
const adminSlice = createSlice({
  ...
  reducers: {
    ...
    state.users = state.users.map(row => row._id === action.payload._id ? action.payload : row)
    },
    // products slice
    ...
    state.products = state.products.map(row => row._id === action.payload.id ? action.payload : row )
    },
    ...
    deleteRecievedNotification: (state, action:PayloadAction<string>) => {
      state.recieved_notifications = state.recieved_notifications.filter(notification => notification._id !== action.payload)
      },
    ...
    addSentNotification:(state, action)=> {
      state.recieved_notifications.push(action.payload);
      // state.NotificationLoading = false;
    },
    deleteSentNotification: (state, action:PayloadAction<string>) => {
      state.sent_notifications = state.sent_notifications.filter(notification => notification._id !== action.payload)
      },
    removeRecipient: (state, action:PayloadAction<recipient>) => {
        const email = action.payload.email;
        const id = action.payload.id;
        const notificationIndex = state.sent_notifications.findIndex(item => item._id === id); 
        if (notificationIndex >= 0) {
          state.sent_notifications[notificationIndex]
          .recipients = state.sent_notifications[notificationIndex]
