...
export const deleteRevNotification = async (req, res, next) => {
  try {
    const deletedNotification = await RecievedNotification.findByIdAndDelete(
      req.params.id
    );
    res
      .status(200)
      .json(`Notification: (${deletedNotification._id}) has been deleted`);
...
export const storeSentNotification = async (req, res, next) => {
  ...
  try {
    ...
    const savedSentNotification = await newSentNotification.save();
    res.status(200).json(savedSentNotification);
  } catch (err) {
    console.log("Err saving to db ", err);
    next(err);
  }
...
export const deleteSentNotification = async (req, res, next) => {
  try {
    const deletedNotification = await SentNotification.findByIdAndDelete(
      req.params.id
    );
    res
      .status(200)
